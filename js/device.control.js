...
// Ini adalah elemen container di mana Anda akan menambahkan elemen tombol on/off
...
export function isiCard(value) {
  ...
  // Buat elemen tombol on/off
  const onOffButton = document.createElement("button");
  onOffButton.className = "on-off-button";
  onOffButton.setAttribute("data-topic", topic);
  ...
  cardDiv.querySelector(".flex-shrink").appendChild(onOffButton);
  devicesContainer.appendChild(cardDiv);

  onOffButton.addEventListener("click", () => {
    const payload = onOffButton.textContent === "On" ? "0" : "1";

    if (mqttClient && mqttClient.connected) {
      mqttClient.publish(topic, payload);
      onOffButton.textContent = payload === "1" ? "On" : "Off";
